...
class QuoteController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        if ($request->filled('quote_number')) {
            $quoteNumber = $request->string('quote_number')->toString();
            $query->where('quote_number', 'like', "%{$quoteNumber}%");
        }

        if ($request->filled('total_ttc') || $request->filled('total_ttc_min') || $request->filled('total_ttc_max')) {
            $exact = trim($request->string('total_ttc')->toString());
            $minRaw = trim($request->string('total_ttc_min')->toString());
            $maxRaw = trim($request->string('total_ttc_max')->toString());

            if ($request->filled('total_ttc') && is_numeric($exact)) {
                $query->where('total_ttc', (float) $exact);
            } else {
                $hasMin = $request->filled('total_ttc_min') && is_numeric($minRaw);
                $hasMax = $request->filled('total_ttc_max') && is_numeric($maxRaw);

                if ($hasMin && $hasMax) {
                    $query->whereBetween('total_ttc', [(float) $minRaw, (float) $maxRaw]);
                } elseif ($hasMin) {
                    $query->where('total_ttc', '>=', (float) $minRaw);
                } elseif ($hasMax) {
                    $query->where('total_ttc', '<=', (float) $maxRaw);
                }
            }
        }
        ...
        return Inertia::render('Quotes/Index', [
            'quotes'  => $quotes,
            'filters' => $request->only(['search', 'quote_number', 'status', 'client_id', 'total_ttc', 'total_ttc_min', 'total_ttc_max']),
    ...
    {
        $duplicateQuote = null;
        $prefillClientId = $request->filled('client_id') ? $request->integer('client_id') : null;
    ...
    public function convertToInvoice(Request $request, Quote $quote, ConvertQuoteToInvoiceAction $convertAction): RedirectResponse
    {
        ...
        try {
            ...
            Log::error('Erreur conversion devis vers facture', $context);
