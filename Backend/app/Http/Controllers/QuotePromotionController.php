...
class QuotePromotionController extends Controller
{
    ...
    public function preview(Request $request, Quote $quote): JsonResponse
    {
        ...
        try {
            ...
            Log::info('Preview promotions pour devis existant', [
                ...
                'user_id' => $userId,
                'has_code' => (bool) $code,
                'promotions_count' => is_array($result['applied_promotions'] ?? null)
                    ? count($result['applied_promotions'])
                    : null,
                'discount_total' => $result['discount_total'] ?? null,
                'grand_total_after' => $result['grand_total_after'] ?? null,
            ]);
            ...
            Log::error('Erreur preview promotions devis', $context);
    ...
    public function apply(Request $request, Quote $quote): JsonResponse
    {
        ...
        try {
            ...
            Log::error('Erreur application promotions devis', $context);
        ...
        try {
            ...
            if (config('app.debug')) {
                Log::info('=== PREVIEW TRANSIENT PROMOTIONS ===', [
                    'user_id' => $userId,
                    'has_code' => (bool) $code,
                    'items_count' => is_array($data['items'] ?? null) ? count($data['items']) : null,
                ]);
            }
            ...
            Log::error('Erreur preview transient promotions', $context);
