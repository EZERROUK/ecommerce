...
class Product extends Model
{
    ...
    public function images(): HasMany                { return $this->hasMany(ProductImage::class)->orderBy('is_primary', 'desc'); }
    ...
    public function approvedReviews(): HasMany       { return $this->hasMany(ProductReview::class)->where('status', ProductReview::STATUS_APPROVED); }
    ...
    /** Alias (si tu veux les utiliser côté front/back) */
    public function compatibleProducts() { return $this->compatibleWith(); }
    public function incomingCompatibleProducts() { return $this->isCompatibleWith(); }
    ...
    public function getAttributesForCategory(): \Illuminate\Support\Collection
    {
        ...
        return $this->category->attributes()
            ->active()
            ->with(['options' => function ($q) { $q->active()->orderBy('sort_order'); }])
            ->orderBy('sort_order')
            ->get()
    ...
    /* -----------------------------------------------------------------
     | Méthodes e-commerce
     |-----------------------------------------------------------------*/
    public function getImageUrl(): ?string    { return $this->image_main ? Storage::url($this->image_main) : null; }
    public function getPrimaryImage(): ?ProductImage { return $this->images()->where('is_primary', true)->first(); }
    ...
    public function hasDiscount(): bool        { return $this->compare_at_price && $this->compare_at_price > $this->price; }
    public function getDiscountPercentage(): ?float
    {
        if (!$this->hasDiscount()) return null;
        return round((($this->compare_at_price - $this->price) / $this->compare_at_price) * 100, 1);
    }
    public function isInStock(): bool          { return !$this->track_inventory || $this->stock_quantity > 0; }
    public function isLowStock(): bool         { return $this->track_inventory && $this->stock_quantity <= $this->low_stock_threshold; }
    public function isAvailable(): bool
    {
        if (!$this->is_active || $this->visibility !== 'public') return false;
    ...
    public function canBeOrdered(): bool       { return $this->isAvailable() && ($this->isInStock() || $this->allow_backorder); }
    ...
    {
        return $this->hasMany(ProductCustomConfig::class, 'product_id');
    }
