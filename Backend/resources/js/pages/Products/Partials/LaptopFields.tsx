        ...
        <div className="space-y-6">
            <div className="mb-4 flex items-center gap-2">
                <div className="flex h-5 w-5 items-center justify-center rounded-sm bg-red-500">
                    <div className="h-2 w-3 rounded-sm border border-white"></div>
                </div>
                <h3 className="text-lg font-medium text-slate-900 dark:text-white">Spécifications Ordinateur Portable</h3>
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm backdrop-blur-md dark:border-slate-700 dark:bg-white/5">
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    {/* CPU */}
                    <div>
                        <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">CPU</label>
                        <input
                            type="text"
                            value={data.cpu ?? ''}
                    ...
                    {/* RAM */}
                    <div>
                        <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">RAM (Go)</label>
                        <input
                            type="number"
                            value={data.ram ?? ''}
                            onChange={(e) => {
                    ...
                    <div>
                        ...
                        <select
                            value={data.keyboard ?? ''}
                            onChange={(e) => setData('keyboard', e.target.value === '' ? undefined : (e.target.value as 'AZERTY' | 'QWERTY'))}
                            className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-slate-900 focus:border-red-500 focus:ring-1 focus:ring-red-500 dark:border-slate-600 dark:bg-slate-800 dark:text-white"
                        >
                    ...
                    {/* Stockage */}
                    <div>
                        <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">Stockage (Go)</label>
                        <input
                            type="number"
                            value={data.storage ?? ''}
                            onChange={(e) => {
                    ...
                    <div>
                        ...
                        <select
                            ...
                            onChange={(e) => setData('storage_type', e.target.value === '' ? undefined : (e.target.value as 'SSD' | 'HDD'))}
                            className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-slate-900 focus:border-red-500 focus:ring-1 focus:ring-red-500 dark:border-slate-600 dark:bg-slate-800 dark:text-white"
                        >
                            <option value="" className="text-slate-500">
                    ...
                    {/* Taille écran */}
                    <div>
                        <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">Taille écran (pouces)</label>
                        <select
                        ...
                        >
                            ...
                            {[11.6, 12.5, 13.3, 14.0, 15.0, 15.6, 16.0, 17.3].map((size) => (
                                <option key={size} value={size}>
                                    {size.toFixed(1)} pouces
                                </option>
                            ))}
                        </select>
                        {errors.screen_size && <div className="mt-1 text-xs text-red-600 dark:text-red-400">{errors.screen_size}</div>}
                    </div>

                    {/* Poids */}
                    <div>
                        <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">Poids (kg)</label>
                        <input
                            type="number"
                            value={data.weight ?? ''}
                            onChange={(e) => {
                            ...
                            placeholder="Ex : 1.35"
                        />
                        {errors.weight && <div className="mt-1 text-xs text-red-600 dark:text-red-400">{errors.weight}</div>}
                    </div>

                    {/* État */}
                    <div className="col-span-1 md:col-span-2">
                        <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">État</label>
                        <select
                ...
                <div className="mt-6 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-900/20">
                    <div className="text-sm text-blue-700 dark:text-blue-300">
                        <p className="mb-1 font-medium">Conseils pour les ordinateurs portables :</p>
