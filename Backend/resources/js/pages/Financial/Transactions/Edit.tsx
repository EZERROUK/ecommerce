...
type FormStatus = 'paid' | 'planned'; // aligné avec la Request
...
export default function EditFinancialTransaction() {
    const { props } = usePage<PageProps>();
    const { transaction, expenseCategories, clients, providers } = props;

    const dueDateTouchedRef = useRef(false);
    const paidAtTouchedRef = useRef(false);

    // On reconstruit une "date" générale pour l'UX à partir de paid_at / due_date / aujourd'hui
    const initialDate = transaction.paid_at || transaction.due_date || new Date().toISOString().slice(0, 10);

    const { data, setData, put, processing, errors, transform } = useForm({
        date: initialDate, // date "visuelle" pour l'UX
        direction: transaction.direction as Direction,
        amount: transaction.amount as unknown as number | string,
        ...
        provider_id: transaction.provider_id ? String(transaction.provider_id) : '',
        status: (transaction.status === 'overdue' || transaction.status === 'canceled' ? 'planned' : transaction.status) as FormStatus,
    ...
    transform((form) => {
        ...
        return {
            ...
            paid_at: isPaid ? form.paid_at || baseDate || null : null,
        };
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('financial.transactions.update', { transaction: transaction.id }));
    };
    ...
    const handleDateChange = (value: string) => {
        ...
        // Sync automatique: si l’utilisateur n’a pas touché les champs spécifiques,
        // la date UX pilote le champ persisté pertinent.
        if (data.status === 'paid') {
            if (!paidAtTouchedRef.current) {
                setData('paid_at', value);
            }
        } else {
    ...
    const handleStatusChange = (value: FormStatus) => {
        setData('status', value);

        if (value === 'paid') {
            if (!paidAtTouchedRef.current) {
                setData('paid_at', data.date);
            }
        } else {
            if (!dueDateTouchedRef.current) {
                setData('due_date', data.date);
            }
        }
    };
    ...
    return (
        <>
            <Head title="Modifier l'opération financière" />
            ...
            <div className="relative min-h-screen bg-gradient-to-br from-white via-slate-100 to-slate-200 transition-colors duration-500 dark:from-[#0a0420] dark:via-[#0e0a32] dark:to-[#1B1749]">
                ...
                <AppLayout
                    breadcrumbs={[
                        ...
                        { title: 'Modifier une opération', href: '#' },
                    ]}
                >
                    <div className="grid grid-cols-12 gap-6 p-6">
                        ...
                        <div className="col-span-12 lg:col-span-8 xl:col-span-8">
                            <div className="rounded-xl border border-slate-200 bg-white p-8 shadow-xl backdrop-blur-md dark:border-slate-700 dark:bg-white/5">
                                <h1 className="mb-2 text-xl font-semibold text-slate-900 dark:text-white">Modifier l&apos;opération financière</h1>
                                <p className="mb-6 text-sm text-slate-600 dark:text-slate-400">
                                    Mets à jour un <span className="font-semibold">encaissement</span> ou un{' '}
                                    <span className="font-semibold">décaissement</span> déjà enregistré pour garder une trésorerie fidèle à la
                                    réalité.
                                </p>
                                ...
                                <form onSubmit={handleSubmit} className="space-y-8">
                                    ...
                                    <section className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                                            ...
                                            <FieldSelect
                                                ...
                                                label="Statut"
                                                value={data.status}
                                                onChange={(v) => handleStatusChange(v as FormStatus)}
                                                options={[
                                    ...
                                    <div className="flex justify-between pt-4">
                                        ...
                                        >
                                            ...
                                            {processing ? 'Enregistrement…' : "Mettre à jour l'opération"}
                                        </Button>
                                    </div>
                                </form>
                            </div>
                                ...
                                <div className="space-y-6 text-sm text-slate-600 dark:text-slate-300">
                                    <div>
                                        ...
                                        <ul className="space-y-1 pl-4">
                                            ...
                                            <li>
                                                • En modifiant une opération, tu ajustes ton <strong>solde net de trésorerie</strong>.
                                            </li>
                                        </ul>
                                    </div>
                                    ...
                                    <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-900/20">
                                        ...
                                        <ul className="space-y-1 pl-4 text-xs">
                                            ...
                                            <li>• Rapports par catégorie, par mois, par fournisseur…</li>
                                        </ul>
                                    </div>
                                    ...
                                    <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-800 dark:bg-emerald-900/20">
                                        ...
                                        <p className="mb-2 text-xs">
                                            En gardant tes opérations à jour, tu améliores la qualité de tes <strong>indicateurs financiers</strong>.
                                        </p>
                                        <ul className="space-y-1 pl-4 text-xs">
