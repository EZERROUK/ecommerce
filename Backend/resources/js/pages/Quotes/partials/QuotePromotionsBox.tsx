...
import { AlertTriangle, Banknote, CheckCircle2, Info, Loader2, Tag, X } from 'lucide-react';
...
export default function QuotePromotionsBox({
    ...
    const discount = useMemo(() => Number(preview?.discount_total ?? 0), [preview?.discount_total]);
    ...
    async function doPreview() {
        ...
        try {
            const data = await postWithCsrfRetry<PreviewResponse>(route('quotes.promotions.preview', quoteId), { code: code?.trim() || null });
            setPreview(data);
    ...
    async function doApply() {
        ...
        try {
            const json = await postWithCsrfRetry<ApplyResponse>(route('quotes.promotions.apply', quoteId), { code: code?.trim() || null });
    ...
    return (
        <div className="rounded-xl border border-slate-200 bg-white p-4 md:p-6 dark:border-slate-700 dark:bg-white/5">
            ...
            <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
                <div className="lg:col-span-2">
                    <div className="relative">
                        <Input placeholder="Code promo (facultatif)" value={code} onChange={(e) => setCode(e.target.value)} className="pr-28" />
                        <div className="absolute top-1/2 right-1 flex -translate-y-1/2 gap-2">
                            <Button size="sm" variant="outline" onClick={doPreview} disabled={loadingPreview}>
                                {loadingPreview ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Prévisualiser'}
                            </Button>
                            <Button size="sm" onClick={doApply} disabled={loadingApply}>
                                {loadingApply ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Appliquer'}
                            </Button>
                        </div>
                    </div>
                ...
                <div className="lg:col-span-1">
                    <div className="rounded-lg border border-slate-200 p-3 dark:border-slate-700">
                        <div className="mb-2 text-xs text-slate-600 dark:text-slate-300">Remise totale estimée</div>
                            ...
                            {list.map((p, idx) => {
                                ...
                                return (
                                    <tr key={`${p.promotion_id}-${idx}`}>
                                        ...
                                        <td className="py-2">
                                            {hasBreakdown ? (
                                                <Badge variant="secondary">{p.lines_breakdown!.length} ligne(s)</Badge>
                                            ) : (
