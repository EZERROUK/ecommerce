...
import { AlertTriangle, BadgeCheck, ChevronLeft, FileText, HeartHandshake, Lock, Mail, RefreshCw, ShieldCheck, UserCheck } from 'lucide-react';
...
const SectionTitle = ({
    ...
    <div id={id} className="flex items-start gap-3">
        <div className="relative">
            ...
            <span className="absolute -top-2 -right-2 inline-flex h-6 min-w-[1.5rem] items-center justify-center rounded-full bg-white px-1.5 text-xs font-semibold text-red-600 ring-1 ring-red-200 dark:bg-slate-900 dark:text-red-400 dark:ring-white/10">
                {number}
            </span>
        </div>
        <div>
...
const PillLink = ({ href, children }: { href: string; children: React.ReactNode }) => (
    <a
        href={href}
        className="rounded-full border border-slate-200 bg-white/70 px-3 py-1.5 text-xs font-medium whitespace-nowrap text-slate-700 shadow-sm hover:text-red-600 dark:border-white/10 dark:bg-white/10 dark:text-slate-200"
    >
        {children}
    </a>
);
...
const ConditionsUtilisation: React.FC<Props> = ({ settings }) => {
    const appName = settings?.app_name ?? 'Système de Gestion Centralisée';
    const contact = settings?.contact_email ?? 'support@globalglimpse.ma';
        ...
        <div className="relative min-h-screen bg-gradient-to-br from-white via-slate-100 to-slate-200 transition-colors duration-500 dark:from-[#0a0420] dark:via-[#0e0a32] dark:to-[#1B1749]">
            ...
            <header className="relative z-10 px-4 pt-12 text-center">
                <div className="mx-auto max-w-3xl">
                    ...
                    <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">Dernière mise à jour : {lastUpdateLabel}</p>
                ...
                <div className="mx-auto max-w-3xl rounded-2xl bg-white/80 p-6 shadow-xl ring-1 ring-slate-200 backdrop-blur dark:bg-white/10 dark:ring-white/10">
                    ...
                    <section className="mt-10 space-y-4">
                        ...
                        <div className="rounded-xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-4 text-sm text-slate-700 shadow-sm dark:border-white/10 dark:from-white/10 dark:to-white/5 dark:text-slate-200">
                            ...
                            <a className="font-semibold text-red-600 underline decoration-dotted hover:text-red-500" href={`mailto:${contact}`}>
                                {contact}
                            </a>
                            .
                        </div>
                    ...
                    <section className="mt-10 space-y-4">
                        ...
                        />
                        <p className="text-sm text-slate-700 dark:text-slate-200">Nous vous recommandons de consulter régulièrement cette page.</p>
                    </section>

                    {/* 8. Contact */}
                    <section className="mt-10 space-y-4">
                        <SectionTitle id="contact" icon={Mail} number="8" title="Contact" subtitle="Une question sur ces conditions ?" />
                        <p className="text-sm text-slate-700 dark:text-slate-200">
                            Écrivez-nous à{' '}
                            <a className="font-semibold text-red-600 underline decoration-dotted hover:text-red-500" href={`mailto:${contact}`}>
                                {contact}
