...
type FilterType = { field: 'user' | 'status' | 'search' | 'date'; value: string };

/* -------------------------------------------------------------------------- */
/* COMPONENTS */
/* -------------------------------------------------------------------------- */
// Composant pour afficher le statut avec style
const StatusDisplay = ({ status }: { status: 'success' | 'failed' }) => {
    ...
    return (
        ...
        >
            ...
            >
                {isSuccess ? 'Connexion réussie' : 'Connexion échouée'}
            </span>
        </div>
    );
};
...
/* COMPONENT */
/* -------------------------------------------------------------------------- */
export default function LoginLogsIndex({ logs, filters = {} }: Props) {
    /* ------------------------------ STATES -------------------------------- */
    ...
    const applyFiltersWithData = (filters: FilterType[]) => {
        ...
        router.get(route('login-logs.index'), payload, inertiaOpts);
    };
    ...
    const resetFilters = () => {
        router.get(route('login-logs.index'), { per_page: logs.per_page, page: 1 }, inertiaOpts);
    };
    ...
    const exportCsv = () => {
        ...
        window.open(route('login-logs.export', payload), '_blank');
    };
            ...
            <div className="relative">
                ...
                <div className="relative z-10 w-full px-4 py-6">
                    ...
                    <div className="mb-6 flex items-center gap-3">
                        <div>
                            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Journal des connexions</h1>
                            <p className="text-sm text-slate-600 dark:text-slate-400">Suivi et traçabilité de toutes les tentatives de connexion</p>
                        </div>
                    </div>
                                                ...
                                                {currentFilterField === 'status' ? (
                                                    <div className="flex gap-2">
                                                        ...
                                                        >
                                                            ...
                                                            <option value="failed">Connexion échouée</option>
                                                        </select>
                                    ...
                                    <tr>
                                        <td colSpan={4} className="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                                            <div className="flex flex-col items-center gap-3">
                                                <LogIn className="h-12 w-12 text-slate-300 dark:text-slate-600" />
                                                <div>
                                            ...
                                            {log.user ? (
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-xs text-slate-500 dark:text-slate-400">Tentative de connexion</div>
                                                    </div>
                                                </div>
                                            )}
                                        </td>
                                                ...
                                                {expanded.includes(log.id) && (
                                                    <div className="mt-2 space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800/50">
                                                        <div className="flex items-center gap-2 text-xs text-slate-700 dark:text-slate-200">
                                                            ...
                                                            <span className="rounded bg-slate-100 px-2 py-0.5 font-mono dark:bg-slate-700">
                                                                {log.ip_address}
                                                            </span>
                                                        </div>

                                                        <div className="flex items-start gap-2 text-xs text-slate-700 dark:text-slate-200">
                                                            <Monitor className="mt-0.5 h-3 w-3 text-slate-400" />
                                                            <div>
                                                                <span className="font-medium">Navigateur:</span>
                                                                <div className="mt-1 max-w-xs break-words text-slate-600 dark:text-slate-300">
                                                                    {log.user_agent}
                                                                </div>
                                                            </div>
                                                        </div>
