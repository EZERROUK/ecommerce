...
export default function CurrenciesIndex({ currencies, flash }: Props) {
    ...
    const filteredCurrencies = useMemo(() => {
        return currenciesArray.filter((currency) =>
            activeFilters.every((f) => {
                if (f.field === 'name') return currency.name.toLowerCase().includes(f.value.toLowerCase());
                if (f.field === 'status') return f.value.toLowerCase() === 'actif' ? !currency.deleted_at : !!currency.deleted_at;
                return true;
            }),
        );
    ...
    const handleDelete = async (code: string) => {
        if (!can('currency_delete')) return alert('Permission manquante: currency_delete');
    ...
    const restoreCurrency = (code: string) => {
        if (!can('currency_restore')) return alert('Permission manquante: currency_restore');
    ...
    const deleteSelectedCurrencies = () => {
        ...
        Promise.all(selectedCurrencyCodes.map((code) => router.delete(route('currencies.destroy', { currency: code }), { preserveScroll: true })))
            .then(() => setSelectedCurrencyCodes([]))
            .catch(() => alert('Une erreur est survenue lors de la suppression de certaines devises.'));
    };
    ...
    const restoreSelectedCurrencies = () => {
        ...
        Promise.all(selectedCurrencyCodes.map((code) => router.post(route('currencies.restore', { id: code }), {}, { preserveScroll: true })))
            .then(() => setSelectedCurrencyCodes([]))
            .catch(() => alert('Une erreur est survenue lors de la restauration de certaines devises.'));
    };

    /* ------------------------- SELECTION HELPERS -------------------------- */
    const toggleSelectCurrency = (code: string) => {
        const currency = currenciesArray.find((c) => c.code === code);
        const isActive = !currency?.deleted_at;

        // on autorise la sélection seulement si l'action correspondante est permise
        if (isActive && !can('currency_delete')) return;
        if (!isActive && !can('currency_restore')) return;
    ...
    const toggleSelectAll = () => {
        ...
        const first = paginatedCurrencies[0];
        const firstActive = !first.deleted_at;

        const canBulkDelete = can('currency_delete');
        const canBulkRestore = can('currency_restore');

        if (selectedCurrencyCodes.length === paginatedCurrencies.length) {
            setSelectedCurrencyCodes([]);
        } else {
            const codes = paginatedCurrencies
                .filter((currency) => {
                    const active = !currency.deleted_at;
                    if (active && !canBulkDelete) return false;
                    if (!active && !canBulkRestore) return false;
                    return active === firstActive;
                })
                .map((c) => c.code);
            setSelectedCurrencyCodes(codes);
        }
    };

    const allSelected = paginatedCurrencies.length > 0 && selectedCurrencyCodes.length === paginatedCurrencies.length;
    const anySelectedInactive = selectedCurrencyCodes.some((code) => currenciesArray.find((c) => c.code === code)?.deleted_at);
    const anySelectedActive = selectedCurrencyCodes.some((code) => !currenciesArray.find((c) => c.code === code)?.deleted_at);

    /* -------------------------- UI Permissions -------------------------- */
    const canCreate = can('currency_create');
    const canBulkDelete = can('currency_delete');
    const canBulkRestore = can('currency_restore');

    // Montrer la colonne "Actions" seulement si au moins une action est possible
    const showActionsColumn = useMemo(() => {
        if (paginatedCurrencies.length === 0) return false;
        return paginatedCurrencies.some((currency) => {
            const isActive = !currency.deleted_at;
            const canShow = can('currency_show');
            const canEdit = can('currency_edit');
            const canDel = can('currency_delete') && isActive;
            const canRes = can('currency_restore') && !isActive;
            return canShow || canEdit || canDel || canRes;
        });
    }, [paginatedCurrencies, can]);

    // Header checkbox visible seulement si une action bulk est permise
    ...
    }, [paginatedCurrencies, canBulkDelete, canBulkRestore]);
                ...
                <div className="relative">
                    ...
                    <div className="relative z-10 w-full px-4 py-6">
                        ...
                        <div className="mb-6 flex items-center gap-3">
                            <div>
                                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Gestion des devises</h1>
                                <p className="text-sm text-slate-600 dark:text-slate-400">Administration et gestion des devises monétaires</p>
                            </div>
                        </div>
                                            ...
                                            <>
                                                {anySelectedInactive && canBulkRestore && (
                                                    <Button variant="secondary" onClick={restoreSelectedCurrencies}>
                                                        <RotateCcw className="mr-1 h-4 w-4" /> Restaurer ({selectedCurrencyCodes.length})
                                                    </Button>
                                                )}
                                                {anySelectedActive && canBulkDelete && (
                                                    <Button variant="destructive" onClick={deleteSelectedCurrencies}>
                                                        <Trash2 className="mr-1 h-4 w-4" /> Supprimer ({selectedCurrencyCodes.length})
                                                    </Button>
                                                )}
                                            </>
                                        )}
                                    ...
                                    {showFilterPanel && (
                                        <div className="w-full rounded-lg border border-slate-200 bg-slate-50 p-4 lg:max-w-xl dark:border-slate-700 dark:bg-slate-800">
                                            <h3 className="mb-3 flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-200">
                                                <SlidersHorizontal className="h-4 w-4" /> Filtrer les devises
                                            </h3>
                                    ...
                                    {canCreate && (
                                        <Link href="/currencies/create">
                                            <Button className="bg-gradient-to-r from-red-600 to-red-500 text-white shadow-md hover:from-red-500 hover:to-red-600">
                                                <Plus className="mr-1 h-4 w-4" /> Ajouter une devise
                                            </Button>
                                        </Link>
                                    )}
                                </div>
                                ...
                                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                    {paginatedCurrencies.length ? (
                                        paginatedCurrencies.map((currency) => {
                                            ...
                                            const canDel = can('currency_delete') && isActive;
                                            const canRes = can('currency_restore') && !isActive;
                                            const canSelectRow = (isActive && can('currency_delete')) || (!isActive && can('currency_restore'));
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    {/* code */}
                                                    <td className="px-6 py-4 text-center">
                                                        <span className="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-800 dark:bg-gray-900/30 dark:text-gray-200">
                                                            {currency.code}
                                                        </span>
                                                    </td>
                                ...
                                <div className="flex items-center gap-2">
                                    <span className="text-slate-600 dark:text-slate-300">
                                        {rowsPerPage === -1
                                            ...
                                            : `Affichage de ${Math.min((currentPage - 1) * rowsPerPage + 1, filteredCurrencies.length)} à ${Math.min(
