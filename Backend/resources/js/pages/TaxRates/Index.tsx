...
export default function TaxRatesIndex({ taxRates, flash }: Props) {
    ...
    const [selectedTaxRateIds, setSelectedTaxRateIds] = useState<number[]>([]);
    ...
    /* ------------------------------ UI Permissions -------------------------- */
    const canCreate = can('taxrate_create');
    const canBulkDelete = can('taxrate_delete');
    const canBulkRestore = can('taxrate_restore');

    const showActionsColumn = useMemo(() => {
        if (!taxRatesArray.length) return false;
        return taxRatesArray.some((t) => {
            const canShow = can('taxrate_show') && !t.deleted_at;
            const canEdit = can('taxrate_edit') && !t.deleted_at;
            const canDel = can('taxrate_delete') && !t.deleted_at;
            const canRes = can('taxrate_restore') && !!t.deleted_at;
    ...
    }, [taxRatesArray, canBulkDelete, canBulkRestore]);
    ...
    const filteredTaxRates = useMemo(() => {
        return taxRatesArray.filter((taxRate) =>
            activeFilters.every((f) => {
                if (f.field === 'name') return taxRate.name.toLowerCase().includes(f.value.toLowerCase());
                if (f.field === 'status') return f.value.toLowerCase() === 'actif' ? !taxRate.deleted_at : !!taxRate.deleted_at;
                return true;
            }),
        );
    ...
    const sortedTaxRates = useMemo(() => {
        return [...filteredTaxRates].sort((a, b) => {
            if (sortField === 'status') {
    ...
    }, [filteredTaxRates, sortField, sortDirection]);

    const paginatedTaxRates = useMemo(() => {
        if (rowsPerPage === -1) return sortedTaxRates;
        const start = (currentPage - 1) * rowsPerPage;
        return sortedTaxRates.slice(start, start + rowsPerPage);
    }, [sortedTaxRates, currentPage, rowsPerPage]);

    const totalPages = useMemo(
        () => (rowsPerPage === -1 ? 1 : Math.ceil(filteredTaxRates.length / rowsPerPage)),
        [filteredTaxRates.length, rowsPerPage],
    );
    ...
    const windowPages = useMemo(() => {
        ...
        if (totalPages <= win + 2) {
            ...
            out.push(1);
            let s = Math.max(2, currentPage - Math.floor(win / 2));
            let e = s + win - 1;
            if (e >= totalPages) {
                e = totalPages - 1;
                s = Math.max(2, e - win + 1);
            }
            if (s > 2) out.push('…');
    ...
    const handleDelete = async (id: number) => {
        if (!can('taxrate_delete')) return alert('Permission manquante: taxrate_delete');
        if (!confirm('Voulez-vous vraiment supprimer ce taux de TVA ?')) return;
        try {
            await router.delete(route('taxrates.destroy', { id }), { preserveScroll: true });
            setSelectedTaxRateIds((p) => p.filter((tid) => tid !== id));
        } catch {
            alert('Une erreur est survenue lors de la suppression.');
        }
    };

    const restoreTaxRate = (id: number) => {
        if (!can('taxrate_restore')) return alert('Permission manquante: taxrate_restore');
        if (!confirm('Voulez-vous restaurer ce taux de TVA ?')) return;
        router.post(route('taxrates.restore', { id }), {}, { preserveScroll: true });
    };

    const deleteSelectedTaxRates = () => {
        if (!canBulkDelete) return alert('Permission manquante: taxrate_delete');
        if (!selectedTaxRateIds.length) return;
        if (!confirm(`Supprimer ${selectedTaxRateIds.length} taux de TVA ?`)) return;
        Promise.all(selectedTaxRateIds.map((id) => router.delete(route('taxrates.destroy', { id }), { preserveScroll: true })))
            .then(() => setSelectedTaxRateIds([]))
            .catch(() => alert('Une erreur est survenue lors de la suppression de certains taux de TVA.'));
    };

    const restoreSelectedTaxRates = () => {
        if (!canBulkRestore) return alert('Permission manquante: taxrate_restore');
        if (!selectedTaxRateIds.length) return;
        if (!confirm(`Restaurer ${selectedTaxRateIds.length} taux de TVA ?`)) return;
        Promise.all(selectedTaxRateIds.map((id) => router.post(route('taxrates.restore', { id }), {}, { preserveScroll: true })))
            .then(() => setSelectedTaxRateIds([]))
            .catch(() => alert('Une erreur est survenue lors de la restauration de certains taux de TVA.'));
    };

    /* ------------------------- SELECTION HELPERS -------------------------- */
    const toggleSelectTaxRate = (id: number) => {
        const row = taxRatesArray.find((r) => r.id === id);
        const isActive = !row?.deleted_at;
        if (isActive && !canBulkDelete) return; // pas le droit de supprimer
        if (!isActive && !canBulkRestore) return; // pas le droit de restaurer

        setSelectedTaxRateIds((p) => (p.includes(id) ? p.filter((r) => r !== id) : [...p, id]));
    };

    const toggleSelectAll = () => {
        if (!paginatedTaxRates.length) return;
        if (selectedTaxRateIds.length) {
            setSelectedTaxRateIds([]);
            return;
        }
        const ids = paginatedTaxRates
            .filter((r) => {
                const isActive = !r.deleted_at;
                if (isActive && !canBulkDelete) return false;
                if (!isActive && !canBulkRestore) return false;
                return true;
            })
            .map((r) => r.id);
        setSelectedTaxRateIds(ids);
    };

    const allSelected =
        paginatedTaxRates.length > 0 &&
        selectedTaxRateIds.length ===
            paginatedTaxRates.filter((r) => {
                const isActive = !r.deleted_at;
                if (isActive && !canBulkDelete) return false;
                if (!isActive && !canBulkRestore) return false;
                return true;
            }).length &&
        paginatedTaxRates.every((r) =>
            !r.deleted_at ? selectedTaxRateIds.includes(r.id) || !canBulkDelete : selectedTaxRateIds.includes(r.id) || !canBulkRestore,
        );

    const anySelectedInactive = selectedTaxRateIds.some((id) => taxRatesArray.find((r) => r.id === id)?.deleted_at);
    const anySelectedActive = selectedTaxRateIds.some((id) => !taxRatesArray.find((r) => r.id === id)?.deleted_at);
                ...
                <div className="relative">
                    ...
                    <div className="relative z-10 w-full px-4 py-6">
                        ...
                        <div className="mb-6 flex items-center gap-3">
                            <div>
                                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Gestion des taux de TVA</h1>
                                <p className="text-sm text-slate-600 dark:text-slate-400">Administration et gestion des taux de taxation</p>
                            </div>
                        </div>
                            ...
                            <div className="flex flex-wrap justify-between gap-4">
                                ...
                                <div className="flex w-full flex-col gap-4 lg:w-auto">
                                    <div className="flex items-center gap-3">
                                        ...
                                        {selectedTaxRateIds.length > 0 && (
                                            <>
                                                {anySelectedInactive && canBulkRestore && (
                                                    <Button variant="secondary" onClick={restoreSelectedTaxRates}>
                                                        <RotateCcw className="mr-1 h-4 w-4" /> Restaurer ({selectedTaxRateIds.length})
                                                    </Button>
                                                )}
                                                {anySelectedActive && canBulkDelete && (
                                                    <Button variant="destructive" onClick={deleteSelectedTaxRates}>
                                                        <Trash2 className="mr-1 h-4 w-4" /> Supprimer ({selectedTaxRateIds.length})
                                                    </Button>
                                                )}
                                            </>
                                        )}
                                    ...
                                    {showFilterPanel && (
                                        <div className="w-full rounded-lg border border-slate-200 bg-slate-50 p-4 lg:max-w-xl dark:border-slate-700 dark:bg-slate-800">
                                            <h3 className="mb-3 flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-200">
                                                <SlidersHorizontal className="h-4 w-4" /> Filtrer les taux de TVA
                                            </h3>
                                    ...
                                    {canCreate && (
                                        <Link href="/tax-rates/create">
                                            <Button className="bg-gradient-to-r from-red-600 to-red-500 text-white shadow-md hover:from-red-500 hover:to-red-600">
                                                <Plus className="mr-1 h-4 w-4" /> Ajouter un taux
                                            </Button>
                                        </Link>
                                    )}
                                </div>
                        ...
                        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl backdrop-blur-md dark:border-slate-700 dark:bg-white/5">
                            <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
                                ...
                                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                    {paginatedTaxRates.length ? (
                                        paginatedTaxRates.map((taxRate) => {
                                            const isActive = !taxRate.deleted_at;
                                            const canShow = can('taxrate_show') && isActive;
                                            const canEdit = can('taxrate_edit') && isActive;
                                            const canDel = can('taxrate_delete') && isActive;
                                            const canRes = can('taxrate_restore') && !isActive;
                                            const canSelectRow = (isActive && canBulkDelete) || (!isActive && canBulkRestore);
                                            const showRowActions = canShow || canEdit || canDel || canRes;

                                            return (
                                                <tr
                                                    key={taxRate.id}
                                                    className={`${taxRate.deleted_at ? 'bg-red-50 dark:bg-red-900/10' : ''} transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50`}
                                                >
                                                    {/* checkbox */}
                                                    {showCheckboxHeader && (
                                                        <td className="px-6 py-4 text-center">
                                                            {canSelectRow ? (
                                                                <input
                                                                    ...
                                                                    onChange={() => toggleSelectTaxRate(taxRate.id)}
                                                                    className="rounded border-slate-300 text-red-600"
                                                                />
                                                            ) : (
                                                    ...
                                                    {/* name */}
                                                    <td className="px-6 py-4 font-medium text-slate-900 dark:text-white">{taxRate.name}</td>
                                                        ...
                                                        <td className="px-6 py-4 text-center">
                                                            {showRowActions ? (
                                                                <div className="flex justify-center gap-2">
                                                                    {canShow && (
                                                                        <Link
                                                                            href={route('taxrates.show', taxRate.id)}
                                                                    ...
                                                                    {canEdit && (
                                                                        <Link
                                                                            href={route('taxrates.edit', taxRate.id)}
                                                                    ...
                                                                    {canRes && (
                                                                        <button
                                                                            onClick={() => restoreTaxRate(taxRate.id)}
                                        ...
                                        <tr>
                                            <td
                                                colSpan={showActionsColumn ? (showCheckboxHeader ? 5 : 4) : showCheckboxHeader ? 4 : 3}
                                                className="px-6 py-12 text-center text-slate-500 dark:text-slate-400"
                                            >
                                                <div className="flex flex-col items-center gap-3">
                                                    <Calculator className="h-12 w-12 text-slate-300 dark:text-slate-600" />
                                                    <div>
                                                        <p className="font-medium">Aucun taux de TVA trouvé</p>
                                                        <p className="text-xs">Aucun taux ne correspond aux critères de recherche</p>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
                            ...
                            <div className="flex flex-col items-center justify-between gap-4 border-t border-slate-200 bg-slate-50 px-6 py-4 text-sm text-slate-700 sm:flex-row dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
                                <div className="flex items-center gap-2">
                                    <span className="text-slate-600 dark:text-slate-300">
                                        {rowsPerPage === -1
                                            ? `Affichage de tous les ${filteredTaxRates.length} résultats`
                                            : `Affichage de ${Math.min((currentPage - 1) * rowsPerPage + 1, filteredTaxRates.length)} à ${Math.min(
                                                  currentPage * rowsPerPage,
                                                  filteredTaxRates.length,
                                              )} sur ${filteredTaxRates.length} résultats`}
                                    </span>
                                </div>
