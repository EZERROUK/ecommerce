...
import { ArrowLeft, CheckCircle2, ChevronDown, Copy, Eye, EyeOff, Mail, Shield, User, UserCog, UserPlus, Wand2, XCircle } from 'lucide-react';
...
export default function CreateUser({ roles }: Props) {
    ...
    const submit = (e: React.FormEvent) => {
        ...
        if (!pwdOK(data.password)) return setPwdErr('10 car. min. + maj + min + chiffre');
        if (data.password !== data.password_confirmation) return setPwdErr('Les mots de passe ne correspondent pas');
                    ...
                    <div className="grid grid-cols-12 gap-6 p-6">
                        ...
                        <div className="col-span-12 lg:col-span-8 xl:col-span-7">
                            <div className="rounded-xl border border-slate-200 bg-white p-8 shadow-xl backdrop-blur-md dark:border-slate-700 dark:bg-white/5">
                                <h1 className="mb-6 text-xl font-semibold text-slate-900 dark:text-white">Nouvel utilisateur</h1>
                                ...
                                <form onSubmit={submit} className="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            <UserCog className="pointer-events-none absolute top-1/2 left-3 h-5 w-5 -translate-y-1/2 text-slate-400" />
                                            ...
                                            <select
                                                ...
                                                className={`block w-full appearance-none rounded-lg border bg-white py-3 pr-10 pl-10 dark:bg-slate-800 ${errors.role ? 'border-red-500 text-red-500' : 'border-slate-300 text-slate-900 dark:border-slate-700 dark:text-white'} focus:border-red-500 focus:ring-1 focus:ring-red-500`}
                                            >
                                    ...
                                    {/* Checklist dynamique */}
                                    <PasswordChecklist showOK={allCriteriaOK} criteria={criteria} />
                                    ...
                                    <div className="flex justify-between pt-4">
                                        ...
                                        <Button
                                            type="submit"
                                            disabled={processing}
                                            className="group relative flex items-center justify-center rounded-lg bg-gradient-to-r from-red-600 to-red-500 px-6 py-3 text-sm font-semibold text-white shadow-md transition-all hover:from-red-500 hover:to-red-600 focus:ring-2 focus:ring-red-500 disabled:opacity-60"
                                        >
                                            {processing ? (
                        ...
                        <div className="col-span-12 lg:col-span-4 xl:col-span-5">
                            <div className="rounded-xl border border-slate-200 bg-white p-8 shadow-xl backdrop-blur-md dark:border-slate-700 dark:bg-white/5">
                                <h2 className="mb-4 text-lg font-medium text-slate-900 dark:text-white">Bonnes pratiques de sécurité</h2>
        ...
        <div>
            ...
            <div className="relative">
                ...
                <input
                    ...
                    className={`block w-full rounded-lg border bg-white py-3 pr-3 pl-10 dark:bg-slate-800 ${error ? 'border-red-500 text-red-500' : 'border-slate-300 text-slate-900 dark:border-slate-700 dark:text-white'} focus:border-red-500 focus:ring-1 focus:ring-red-500`}
                />
            </div>
        ...
        <div>
            ...
            <div className="relative">
                ...
                <input
                    ...
                    onChange={(e) => onChange(e.target.value)}
                    className={`block w-full rounded-lg border bg-white py-3 pr-24 pl-10 dark:bg-slate-800 ${error ? 'border-red-500 text-red-500' : 'border-slate-300 text-slate-900 dark:border-slate-700 dark:text-white'} focus:border-red-500 focus:ring-1 focus:ring-red-500`}
                    aria-invalid={!!error}
                ...
                <div className="absolute inset-y-0 right-2 flex items-center gap-1">
                    {onGenerate && (
                        <button
                            type="button"
                            onClick={onGenerate}
                            className="rounded-md p-1.5 hover:bg-slate-100 dark:hover:bg-slate-700/60"
                            title="Proposer un mot de passe fort"
                        >
...
function PasswordChecklist({ criteria, showOK }: { criteria: { length: boolean; lower: boolean; upper: boolean; digit: boolean }; showOK: boolean }) {
    ...
    const Row = ({ ok, label }: { ok: boolean; label: string }) => (
        <li className={`flex items-center gap-2 ${ok ? 'text-slate-500 line-through dark:text-slate-400' : 'text-red-500'}`}>
    ...
    return (
        <div className="rounded-lg border border-red-200 bg-red-50/70 p-3 dark:border-red-900/50 dark:bg-red-900/10">
            <p className="mb-1 text-sm font-medium text-red-600 dark:text-red-400">Critères requis :</p>
