...
export const SUBJECT_TYPES = {
    ...
    'App\\Models\\FinancialTransaction': 'Transaction',
...
export function getSubjectTypeName(subjectType: string): string {
    return SUBJECT_TYPES[subjectType as keyof typeof SUBJECT_TYPES] || subjectType.split('\\').pop() || subjectType;
}

// Fonction pour traduire une description d'action
export function translateActionDescription(description: string, subjectType: string, event?: string | null): string {
    const subjectName = getSubjectTypeName(subjectType);

    // Si l’event est présent (ex: Eloquent created/updated/...), on s’en sert
    // pour avoir un libellé court et cohérent.
    const normalizedEvent = String(event ?? '')
        .trim()
        .toLowerCase();
    if (['created', 'updated', 'deleted', 'restored'].includes(normalizedEvent)) {
        return `${subjectName} ${normalizedEvent}`;
    }
...
export function createBilingualSearchTerms(searchTerm: string): string[] {
    ...
    for (const [key, value] of Object.entries(SUBJECT_TYPES)) {
        if (key.toLowerCase().includes(searchTerm.toLowerCase()) || value.toLowerCase().includes(searchTerm.toLowerCase())) {
            terms.push(key);
            terms.push(value);
        }
    }
...
export function getActionColor(description: string): string {
    ...
    if (lowerDesc.includes('créé') || lowerDesc.includes('created') || lowerDesc.includes('ajouté') || lowerDesc.includes('added')) {
        return 'text-emerald-600 dark:text-emerald-400';
    }

    if (lowerDesc.includes('modifié') || lowerDesc.includes('updated') || lowerDesc.includes('mis à jour') || lowerDesc.includes('changed')) {
        return 'text-blue-600 dark:text-blue-400';
    }

    if (lowerDesc.includes('supprimé') || lowerDesc.includes('deleted') || lowerDesc.includes('retiré') || lowerDesc.includes('removed')) {
        return 'text-red-600 dark:text-red-400';
    }

    if (lowerDesc.includes('connecté') || lowerDesc.includes('logged') || lowerDesc.includes('accédé') || lowerDesc.includes('accessed')) {
        return 'text-purple-600 dark:text-purple-400';
    }

    if (lowerDesc.includes('approuvé') || lowerDesc.includes('approved') || lowerDesc.includes('validé') || lowerDesc.includes('validated')) {
        return 'text-teal-600 dark:text-teal-400';
    }

    if (lowerDesc.includes('rejeté') || lowerDesc.includes('rejected') || lowerDesc.includes('annulé') || lowerDesc.includes('cancelled')) {
...
export function getActionIcon(description: string): string {
    ...
    if (lowerDesc.includes('consulté') || lowerDesc.includes('viewed') || lowerDesc.includes('vu')) {
        return 'Eye';
    }

    if (lowerDesc.includes('envoyé') || lowerDesc.includes('sent') || lowerDesc.includes('expédié')) {
        return 'Send';
    }

    if (lowerDesc.includes('téléchargé') || lowerDesc.includes('downloaded') || lowerDesc.includes('uploaded')) {
        return 'Download';
    }

    if (lowerDesc.includes('approuvé') || lowerDesc.includes('approved') || lowerDesc.includes('validé')) {
        return 'Check';
    }

    if (lowerDesc.includes('rejeté') || lowerDesc.includes('rejected') || lowerDesc.includes('annulé')) {
        return 'X';
    }
