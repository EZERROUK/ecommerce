...
export default function DynamicAttributeField({ attribute, value, onChange, error }: DynamicAttributeFieldProps) {
    ...
    const renderField = () => {
        switch (type) {
                            ...
                            {activeOptions.map((option) => (
                                <SelectItem key={option.id} value={option.value}>
                                    <div className="flex items-center gap-2">
                                        {option.color && <div className="h-3 w-3 rounded-full border" style={{ backgroundColor: option.color }} />}
                                        {option.label}
                                    </div>
                                </SelectItem>
                            ))}
            ...
            case 'multiselect': {
                const selectedValues = Array.isArray(value) ? value : typeof value === 'string' && value ? value.split(',') : [];
                                ...
                                {activeMultiOptions
                                    ...
                                    .map((option) => (
                                        <SelectItem key={option.id} value={option.value}>
                                            <div className="flex items-center gap-2">
                                                {option.color && (
                                                    <div className="h-3 w-3 rounded-full border" style={{ backgroundColor: option.color }} />
                                                )}
                                                {option.label}
                                            </div>
                                        </SelectItem>
                                ...
                                {selectedValues.map((val) => {
                                    ...
                                    return (
                                        <Badge key={val} variant="secondary" className="flex items-center gap-1">
                                            {option?.color && <div className="h-2 w-2 rounded-full" style={{ backgroundColor: option.color }} />}
                                            {option?.label || val}
                                            <button
                                                type="button"
                                                onClick={() => {
