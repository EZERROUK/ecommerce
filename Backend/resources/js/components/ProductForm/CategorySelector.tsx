...
export default function CategorySelector({ categories, value, onChange, error, disabled = false }: CategorySelectorProps) {
    ...
    }, [value, categories, fetchCategoryAttributes, onChange]);
            ...
            <div className="text-muted-foreground mt-2 flex items-center gap-1 text-sm">
                {breadcrumb.map((item, index) => (
                    <React.Fragment key={index}>
                        ...
                        <span className={index === breadcrumb.length - 1 ? 'text-foreground font-medium' : ''}>{item}</span>
                    </React.Fragment>
                ))}
            </div>
        );
    ...
    return (
        <div className="space-y-4">
            <div className="space-y-2">
                ...
                <Select value={value?.toString() || ''} onValueChange={handleCategoryChange} disabled={disabled}>
                    ...
                    <SelectContent>
                        {categories.map((category) => (
                            ...
                            >
                                <span className={category.has_children ? 'font-medium' : ''}>{category.indented_name}</span>
                                {category.has_children && <span className="text-muted-foreground ml-2 text-xs">(contient des sous-catégories)</span>}
                            </SelectItem>
                        ))}
                    </SelectContent>
                </Select>
            ...
            {selectedCategory && selectedCategory.has_children && (
                <Alert>
                    ...
                    <AlertDescription>
                        Cette catégorie contient des sous-catégories. Veuillez sélectionner une catégorie finale pour définir les attributs du
                        produit.
                    </AlertDescription>
                </Alert>
            )}
            ...
            {selectedCategory && !selectedCategory.has_children && (
                <div className="bg-muted/50 space-y-4 rounded-lg border p-4">
                    <div className="flex items-center gap-2">
                        ...
                        {loading && <div className="border-primary h-4 w-4 animate-spin rounded-full border-2 border-t-transparent" />}
                    </div>

                    {attributes.length === 0 && !loading && (
                        <p className="text-muted-foreground text-sm">Aucun attribut défini pour cette catégorie.</p>
                    )}

                    {attributes.length > 0 && (
                        <div className="text-muted-foreground text-sm">
                            {attributes.length} attribut{attributes.length > 1 ? 's' : ''} trouvé{attributes.length > 1 ? 's' : ''}. Les champs
                            apparaîtront dans l'onglet "Attributs".
                        </div>
                    )}
                </div>
            )}
