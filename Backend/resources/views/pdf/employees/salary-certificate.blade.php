...
@section('document_title', 'Attestation de Salaire - ' . $employee->full_name)
@section('ref_prefix', 'AS')
...
@section('main')
    <div class="doc-title">
        @if(($lang ?? 'fr') === 'ar')
            <span class="rtl">شهادة الراتب</span>
        @else
    ...
    <div class="avoid-break">
        <div class="block">
            <div class="h2">Salaire déclaré</div>
            <table class="kv">
                <tr>
                    <td class="label">Période</td>
                    <td class="value">{{ $issueDate->locale(($lang ?? 'fr'))->isoFormat('MMMM YYYY') }}</td>
                </tr>
                <tr>
                    <td class="label">Montant brut</td>
                    <td class="value">
                ...
                @if($employee->bonus_target)
                <tr>
                    <td class="label">Prime / Bonus</td>
                    <td class="value">{{ number_format($employee->bonus_target, 2, ',', ' ') }} {{ $employee->salary_currency ?? 'MAD' }}</td>
                </tr>
                @endif
            </table>
        </div>
